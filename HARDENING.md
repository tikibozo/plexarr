...
This doc explains the patterns and the *why* behind each one — if you're copying anything from this repo into your own stack, this is the file to read alongside the compose files themselves.
...
- **LSIO 5-cap baseline** — for [linuxserver.io](https://www.linuxserver.io/) images and anything else that uses an s6/gosu init to chown `/config` and drop to a non-root user. The init needs:
    ...
    cap_add:
      ...
      - SETGID
    ```
    Some images need a 6th (`NET_BIND_SERVICE` for Jellyfin's DLNA stack binding UDP 1900 inside the container; `KILL` for services that signal child processes).
...
The reverse problem: services on `internal: true` networks **cannot resolve host DNS** (which made my `unpackerr` config silently break for 36 hours during Phase 6c when I left `tomservo.mylocal:N` URLs in the env vars after flipping `arr_control` to internal). Use **container names**, not host DNS, for cross-service URLs once a zone is internal.
